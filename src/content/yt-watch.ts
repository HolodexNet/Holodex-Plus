import { Options, waitForEl } from "src/util";
(async () => {
  // Abort if turned off
  if (!(await Options.get("openInHolodexButton"))) return;
  
  // Try to insert button on navigating to watch page?
  window.addEventListener('yt-navigate-start', tryCreateButton, true);

  // Or try if directly loaded into a watch page
  ...
  async function tryCreateButton() {
    // Already exists
    if(document.querySelector(".yt-watch-holodex-btn")) return;

    const element = await waitForEl(".ytd-video-primary-info-renderer #top-level-buttons-computed");
    const icon = `
